    ...
    <div className="experience__card">
      ...
      <div className="experience__card-bottom">
        ...
        {item.links.map((link: { item: string; image: string },index:number)=>
          <Link to={link.item} className="experience__card-links" style={{"backgroundImage": `url("${process.env.REACT_APP_BASE_URL}/img/${link.image}.png")`}} key={index} target='_blank'/>
        )}
      </div>
      }
    </div>
